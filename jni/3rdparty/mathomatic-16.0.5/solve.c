 ...
 */
int
#if defined(MODIFY_FUNCTION_DEFINED)
solve_espace(int want, int have)
#else
...
#ifdef USE_EXTERN_STRING
		debug_string(0,GetCmdOperateText(eCmdOpSolveSuccessful));
#else
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
solve_sub(token_type	*wantp,int wantn, token_type	*leftp, int		*leftnp, token_type	*rightp, int		*rightnp)
#else
...
#ifdef USE_EXTERN_STRING
		debug_string(1, GetCmdOperateText(eCmdOpSolvingForZero));
#else
...
see_work:
...
#ifdef USE_EXTERN_STRING
		debug_string(1, GetCmdOperateText(eCmdOpSolveVariableToRHS));
#else
	...
	if (se_compare(wantp, wantn, tlhs, n_tlhs, &diff_sign) && !diff_sign) {
		if (zsolve) {
#ifdef USE_EXTERN_STRING
			debug_string(1, GetCmdOperateText(eCmdOpSimplifyingZeroSolve));
#else
...
#ifdef USE_EXTERN_STRING
			debug_string(1, GetCmdOperateText(eCmdOpSolveZeroCompleted));
#else
			debug_string(1, _("Solve for zero completed:"));
#endif
		} else {
#ifdef USE_EXTERN_STRING
			debug_string(1, GetCmdOperateText(eCmdOpSolveCompleted));
#else
		...
		if (p1 >= ep || (p1->level == 1 && p1->kind == OPERATOR)) {
			...
			if (p1 >= ep) {
...
#ifdef USE_EXTERN_STRING
						debug_string(0, GetCmdOperateText(eCmdOpLHSidenticalToRHS));
#else
				...
				if (worked && !uf_flag) {
					worked = false;
#ifdef USE_EXTERN_STRING
					debug_string(1, GetCmdOperateText(eCmdOpUnfactoring));
#else
...
#if	!SILENT
#ifdef USE_EXTERN_STRING
						fprintf(gfp,GetCmdOperateText(eCmdOpRemovingPossible));
#else
...
#ifdef USE_EXTERN_STRING
						debug_string(1, GetCmdOperateText(eCmdOpJuggling));
#else
...
#endif
{
	...
	if (debug_level >= 0) {
#ifdef USE_EXTERN_STRING
		fprintf(gfp, GetCmdOperateText(eCmdOpRaisingBothPowerExp), precision, 1.0 / d);
#else
...
#endif
{
...
#ifdef USE_EXTERN_STRING
	debug_string(1, GetCmdOperateText(eCmdOpCheckingEquation));
#else
	...
	if (debug_level >= 0) {
		list_var(v, 0);
#ifdef USE_EXTERN_STRING
		fprintf(gfp,GetCmdOperateText(eCmdOpEquationIsN_Degree),var_str,precision, high_power);
#else
...
big_bbreak:
	...
	list_tdebug(1);
#ifdef USE_EXTERN_STRING
	debug_string(0, GetCmdOperateText(eCmdOpEquationSolved));
#else
...
#endif
{
	...
	if (*side1np == prev_n1 && *side2np == prev_n2) {
		if (++repeat_count >= 4) {
#ifdef USE_EXTERN_STRING
			debug_string(1, GetCmdOperateText(eCmdOpInfiniteLoop));
#else
	...
	if (op == POWER && operandp == side1p) {
		...
		if (!parse_complex(operandp, operandn, &c2))
			return false;
#ifdef USE_EXTERN_STRING
		debug_string(1, GetCmdOperateText(eCmdOpTakingLogarithm));
#else
	...
	if (debug_level > 0) {
		...
		case PLUS:
#ifdef USE_EXTERN_STRING
			fprintf(gfp, GetCmdOperateText(eCmdOpSubtracting));
#else
		...
		case MINUS:
#ifdef USE_EXTERN_STRING
			fprintf(gfp, GetCmdOperateText(eCmdOpAdding));
#else
		...
		case TIMES:
#ifdef USE_EXTERN_STRING
			fprintf(gfp, GetCmdOperateText(eCmdOpDividing));
#else
		...
		case DIVIDE:
#ifdef USE_EXTERN_STRING
			fprintf(gfp, GetCmdOperateText(eCmdOpMultiplying));
#else
		...
		case POWER:
#ifdef USE_EXTERN_STRING
			fprintf(gfp, GetCmdOperateText(eCmdOpRaisingBothPower));
#else
		...
		if (op == POWER && operandn == 1 && operandp->kind == CONSTANT) {
...
#ifdef USE_EXTERN_STRING
			fprintf(gfp, GetCmdOperateText(eCmdOpBracketsLeft));
#else
			fprintf(gfp, " \"");
#endif
			if (op == POWER)
			...
			case PLUS:
#ifdef USE_EXTERN_STRING
				fprintf(gfp, GetCmdOperateText(eCmdOpTheResult01));
#else
...
static int
#if defined(MODIFY_FUNCTION_DEFINED)
flip(token_type	*side1p, int		*side1np, token_type	*side2p, int		*side2np)
#else
...
#endif
{
	token_type	*p1, *ep;
#ifdef USE_EXTERN_STRING
	debug_string(1,GetCmdOperateText(eCmdOpTakingReciprocal));
#else
