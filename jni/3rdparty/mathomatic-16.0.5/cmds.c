...
static int
#if defined(MODIFY_FUNCTION_DEFINED)
sum_product(char	*cp, enum spf_function current_function)
#else
...
static int
#if defined(MODIFY_FUNCTION_DEFINED)
find_more(token_type	*equation, int		*np, int en)
#else
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
output_current_directory(FILE	*ofp)
#else
...
void
#if defined(MODIFY_FUNCTION_DEFINED)
output_options(FILE	*ofp, int all_set_options)
#else
...
static int
#if defined(MODIFY_FUNCTION_DEFINED)
complex_func(char	*cp, int imag_flag)
#else
...
#endif
{
	...
	if (solved) {
		...
		fprintf(gfp,GetCmdOperateText(eCmdOpEliminatingVariable),i+1,var_str);
#else
...
#endif
{
	...
	do {
		...
		for (i = first; i <= last; i++) {
			...
			if (counter_max) {
#ifdef USE_EXTERN_STRING
				fprintf(gfp, GetCmdOperateText(eCmdOpPossibleSolutions), counter_max + 1);
#else
...
#ifdef USE_EXTERN_STRING
							fprintf(gfp, GetCmdOperateText(eCmdOpSolutions), counter + 1);
#else
...
#ifdef USE_EXTERN_STRING
					fprintf(gfp, " ");
					fprintf(gfp, GetCmdOperateText(eCmdOpApplyingSolutions));
#else
					fprintf(gfp, ":\n");
#endif
				}
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
read_sub(FILE	*fp, char	*filename)
#else
