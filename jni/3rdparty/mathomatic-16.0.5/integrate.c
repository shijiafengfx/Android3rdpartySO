...
make_powers(token_type	*equation, int		*np, long v)
#else
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
int_dispatch(token_type	*equation, int		*np, long v, int (*func)(token_type *equation, int *np, int loc, int eloc, long v))
#else
...
static int
#if defined(MODIFY_FUNCTION_DEFINED)
integrate_sub(token_type	*equation, int		*np, int loc, int eloc,long v)
#else
