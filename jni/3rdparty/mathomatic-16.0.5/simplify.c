...
static int
#if defined(MODIFY_FUNCTION_DEFINED)
org_recurse(token_type	*equation, int		*np, int loc,int level,int *elocp)
#else
...
void
#if defined(MODIFY_FUNCTION_DEFINED)
simp_ssub(token_type	*equation, int		*np, long v, double d,int power_flag,int times_flag,int fc_level)
#else
...
void
#if defined(MODIFY_FUNCTION_DEFINED)
mid_simp_side(token_type	*equation, int		*np)
#else
...
void
#if defined(MODIFY_FUNCTION_DEFINED)
simpv_side(token_type	*equation, int		*np, long v)
#else
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
factor_imaginary(token_type	*equation, int		*np)
#else
...
void
#if defined(MODIFY_FUNCTION_DEFINED)
factorv(token_type	*equation, int		*np, long v)
#else
...
static int
#if defined(MODIFY_FUNCTION_DEFINED)
simpb_vcmp(sort_type	*p1, sort_type	*p2)
#else
simpb_vcmp(p1, p2)
sort_type	*p1, *p2;
#endif
...
/* Here we do the greatest expansion; if it fails, do less expansion. */
	...
	debug_string(2, GetCmdOperateText(eCmdOpSimplifyResult));
	side_debug(2, equation, *np);
#else
...
void
#if defined(MODIFY_FUNCTION_DEFINED)
simpa_repeat(int n,int quick_flag,int frac_flag)
#else
...
void
#if defined(MODIFY_FUNCTION_DEFINED)
simple_frac_repeat_side(token_type	*equation,int		* np)
#else
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
simp_loop(token_type	*equation,int		* np)
#else
simp_loop(equation, np)
token_type	*equation;	/* pointer to the beginning of equation side to simplify */
int		*np;		/* pointer to length of equation side */
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
combine_constants(token_type	*equation, int		*np,int iflag)
#else
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
calc(int	*op1p, double	*k1p, int op2, double k2)
#else
...
#else
#if	!_REENTRANT
#if defined(_WIN32) || IS_ANDROID_PLATFORM
#else
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
elim_k(token_type	*equation, int		*np)
#else
elim_k(equation, np)
token_type	*equation;	/* equation side pointer */
int		*np;		/* pointer to length of equation side */
...
int
#if defined(MODIFY_FUNCTION_DEFINED)
se_compare(token_type	*p1,int n1, token_type	*p2,int n2, int		*diff_signp)
#else
				...
				for (k = j - 2; k > i1; k -= 2) {
					if ((equation[k].level == (div_level + 2)
						...
						*np += 2 * (k + 1);
						...
						k1 += k;
#ifdef USE_EXTERN_STRING
						debug_string(2, "Square roots in denominator rationalized:");
						side_debug(2, &equation[i+1], k1 - (i + 1));
#else
